...
function s.initial_effect(c)
	...
	local e2=aux.AddNormalSummonProcedure(c,true,true,1,1,SUMMON_TYPE_TRIBUTE,aux.Stringid(id,1),s.otfilter)
	--summon success
...
function s.otfilter(c,tp,relzone)
	return (c:IsControler(tp) or c:IsFaceup()) and c:IsSetCard(0x2e) and (not relzone or aux.IsZone(c,relzone,tp))
end
function s.atkfilter(c)
	return c:IsSetCard(0x2e) and c:IsLocation(LOCATION_MZONE)
end
function s.valcheck(e,c)
	local g=c:GetMaterial()
	local ct=g:FilterCount(Card.IsSetCard,nil,0x2e)
	local lv=g:Filter(s.atkfilter,nil):GetSum(Card.GetLevel)
	e:SetLabel(lv)
